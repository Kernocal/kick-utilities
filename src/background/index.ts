    ...
    try {
        const res = chrome.tabs.sendMessage(tabID, message, (res) => {
            if (chrome.runtime.lastError) {
                console.warn('Callback failed, tab probably does not exist', chrome.runtime.lastError.message);
            }
            callback(res);
        });
        return res;
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    ...
    if (url.hostname.includes('kick.com')) {
        if (changeInfo.status === 'complete') {
            if (url.pathname.includes('/video') && url.searchParams.has('t')) {
                sendMessage(tabId, {
                    message: 'video', 
                    time: url.searchParams.get('t')
                }, (res: any) => {});
            } else if (url.searchParams.has('clip')) {
                sendMessage(tabId, {
                    message: 'clip', 
